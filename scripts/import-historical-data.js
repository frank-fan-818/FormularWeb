...
async function importSeasons() {
  ...
  try {
    ...
    const filteredSeasons = seasons
      .filter(s => parseInt(s.season) <= MAX_SEASON)
      .map(s => ({
...
async function importConstructors() {
  ...
  try {
    ...
    const formattedConstructors = constructors.map(c => ({
 ...
 * 导入指定赛季的赛历
 */
async function importSeasonRaces(season) {
 ...
 * 导入指定赛季的比赛结果
 */
async function importRaceResults(season, raceIdMap) {
 ...
 * 导入指定赛季的排位赛结果
 */
async function importQualifyingResults(season, raceIdMap) {
